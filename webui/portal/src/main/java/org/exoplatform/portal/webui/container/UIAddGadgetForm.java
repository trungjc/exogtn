...
@ComponentConfig(lifecycle = UIFormLifecycle.class, template = "system:/groovy/webui/form/UIAddGadgetForm.gtmpl", events = @EventConfig(listeners = UIAddGadgetForm.AddGadgetByUrlActionListener.class))
public class UIAddGadgetForm extends UIForm
{

   public static String FIELD_URL = "url";
   
   public static final String RSS_GADGET_ID = "rssAggregator";
   ...
   static public class AddGadgetByUrlActionListener extends EventListener<UIAddGadgetForm>
   {
      public void execute(final Event<UIAddGadgetForm> event) throws Exception
      {
         ...
         UIDashboardLayoutContainer uiDashboard = uiForm.getAncestorOfType(UIDashboardLayoutContainer.class);
         UIColumnContainer column = uiDashboard.getChild(UIColumnContainer.class);
         ...
         {
            uiApplication.addMessage(new ApplicationMessage("UIAddGadgetForm.msg.required", null));
            return;
         }
         url = url.trim();
         if (!url.matches(URLValidator.URL_REGEX))
         {
            uiApplication.addMessage(new ApplicationMessage("UIAddGadgetForm.msg.notUrl", null));
            return;
         }
         Gadget gadget;
         UIGadget uiGadget;
         ...
         {
            String rssId = UIAddGadgetForm.RSS_GADGET_ID;
            gadget = service.getGadget(rssId);
         ...
         column.getChildren().add(0, uiGadget);
